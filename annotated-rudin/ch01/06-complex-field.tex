...
1.18(d),\kn{Exactly the leverage promised at 1.18(d). Squares are nonnegative
and not both $a,b$ vanish, so $a^2+b^2>0$ --- and therefore the divisions below
are legal. The construction of $\C$ depends on an order property of $\R$, which
is worth noticing given that $\C$ itself cannot be ordered (Exercise 1.8).} and
we can define
